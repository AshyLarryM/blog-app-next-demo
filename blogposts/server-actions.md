...
```
- **Finally we want to run code on the server using another server action inside of an async function called "toggleTodos".**.
```
...
```
- This code will now give us the ability to update the toggle of any todo and write as a boolean value in the database, and reflect accordingly inside of our <TodoItem /> component.

- Since we are using an event handler (client interactivity) inside of the <TodoItem> component, the component must be a client component.  At the top of the _TodoItem.tsx_ add in ```'use client'```
